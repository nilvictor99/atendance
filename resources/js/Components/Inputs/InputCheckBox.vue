<script setup>
    ...
    const inputType = computed(() => {
        return 'checkbox'; // Always checkbox
    });
    ...
    function handleInput() {
        ...
        if (props.multi) {
            ...
            // For single checkbox, toggle checked state
            if (isChecked.value) {
                if (props.allowDeselect) {
    ...
    <div>
        ...
        >
            <input
                ...
                @input="handleInput"
                @keydown="onKeyDown"
                class="rounded form-checkbox h-5 w-5 focus:ring-2 transition-all duration-150 ease-in-out transform hover:scale-105"
                :class="[colorClasses, { 'ring-4 ring-opacity-50': isChecked }]"
            />
